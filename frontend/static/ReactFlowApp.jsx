  ...
  const onToggleLandOwner = useCallback((nodeId) => {
    commitLogicalNodes((prev) => {
      ...
      if (!nextIsLandOwner && node) {
        ...
        const anchorHasInflow = Number(node.inheritedShare || 0) > 0 || Number(node.finalShare || 0) > 0 || node.hasInflow;
        const anchorHasDeadChild = nextNodes.some((n) => n.relationType === "child" && n.person?.death);
        const anchorIsDead = !!node.person?.death;
