...
function FamilyTreeApp() {
  ...
  const moveWithinDiagram = useCallback((sourceNodeId, targetNodeId) => {
    ...
    commitLogicalNodes((prev) => {
      ...
      const baseNodes = pruneLinkedNodes(prev, sourceNodeId, { keepTarget: true });
      if (target?.kind === "ghost") {
        return ensureSpareChildNode([
          ...baseNodes,
  ...
  const onToggleLandOwner = useCallback((nodeId) => {
    commitLogicalNodes((prev) => {
      ...
      if (!nextIsLandOwner && node) {
        // Remove auto-generated spouse slots for this anchor if they were created only because of land owner.
        const autoSpouses = nextNodes.filter((n) => n.autoGenerated && n.autoAnchorId === nodeId && n.spouseOf === nodeId && !n.person);
        const occupiedAutoSpouses = nextNodes.filter((n) => n.autoGenerated && n.autoAnchorId === nodeId && n.spouseOf === nodeId && n.person);
        nextNodes = nextNodes.filter((n) => !(n.autoGenerated && n.autoAnchorId === nodeId && n.spouseOf === nodeId && !n.person));
