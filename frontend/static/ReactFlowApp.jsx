...
function resolveSubRelations(nodes, shareMode) {
  ...
  // Determine whether the fixed owner spouse slot should be visible.
  // Show when: owner isLandOwner, OR owner is dead with share, OR there is a dead child.
  const ownerNode = resolvedNodes.find((n) => n.id === "owner");
  const spouseNode = resolvedNodes.find((n) => n.relationType === "spouse" && (n.spouseOf === "owner" || n.id === "spouse"));
  const ownerHasDeadChild = resolvedNodes.some((n) => n.relationType === "child" && n.person?.death);
  const ownerIsDeadWithShare = ownerNode?.person?.death && (Number(ownerNode.inheritedShare || 0) > 0 || Number(ownerNode.finalShare || 0) > 0 || ownerNode.hasInflow);
  const spouseIsDeadWithShare = spouseNode?.person?.death && (Number(spouseNode.inheritedShare || 0) > 0 || Number(spouseNode.finalShare || 0) > 0 || spouseNode.hasInflow);
  const shouldShowOwnerSpouse = !!(ownerNode?.isLandOwner || ownerIsDeadWithShare || ownerHasDeadChild);
  const shouldShowBirthParentSlots = !!(ownerNode?.person && (ownerNode.isLandOwner || ownerNode.person.death || ownerIsDeadWithShare || ownerHasDeadChild));
  const shouldShowSpouseParentSlots = !!(spouseNode?.person && (spouseNode.isLandOwner || spouseNode.person.death || spouseIsDeadWithShare || ownerHasDeadChild));
  resolvedNodes = resolvedNodes.map((n) => {
    if (n.id === "spouse" && !n.autoGenerated && !n.person) {
      return { ...n, hidden: !shouldShowOwnerSpouse };
    }
    if ((n.id === "father" || n.id === "mother") && !n.person) {
      return { ...n, hidden: !shouldShowBirthParentSlots };
    }
    if ((n.id === "spouse_father" || n.id === "spouse_mother") && !n.person) {
      return { ...n, hidden: !shouldShowSpouseParentSlots };
    }
    return n;
  });
  ...
  const usedNodeIds = new Set(resolvedNodes.map((n) => n.id));
  function nextAutoZSlotId(parentNode, existingZCount) {
    const baseId = `auto_z_${parentNode.id}`;
    let suffix = Math.max(1, existingZCount + 1);
    let candidate = suffix === 1 ? baseId : `${baseId}_${suffix}`;
  ...
  resolvedNodes.forEach((node) => {
    ...
    const existingZCount = resolvedNodes.filter((n) =>
      n.kind === "person" &&
    ...
    autoNodes.push(createLogicalNode({
      id: nextAutoZSlotId(node, existingZCount),
      kind: "person",
      label: "",
      role: "Anh/Chị/Em",
      relationType: "sibling",
  ...
  const onToggleLandOwner = useCallback((nodeId) => {
    commitLogicalNodes((prev) => {
      ...
      if (!nextIsLandOwner && node) {
        ...
        const resolvedAfterToggle = resolveSubRelations(nextNodes, shareMode).nodes;
        const anchorAfterToggle = resolvedAfterToggle.find((n) => n.id === nodeId) || node;
        const anchorPersonId = String(anchorAfterToggle.person?.id || node.person?.id || "").trim();
        const anchorHasInflow = Number(anchorAfterToggle.inheritedShare || 0) > 0 || Number(anchorAfterToggle.finalShare || 0) > 0 || anchorAfterToggle.hasInflow;
        const anchorHasDeadChild = resolvedAfterToggle.some((n) =>
          n.kind === "person" &&
          n.person?.death &&
          (
            n.parentSlotId === nodeId ||
            (anchorPersonId && String(n.parentPersonId || "") === anchorPersonId) ||
            (nodeId === "owner" && n.relationType === "child" && n.parentSlotId === "owner") ||
            (anchorAfterToggle.relationType === "spouse" && n.relationType === "child" && n.parentSlotId === "owner")
          )
        );
        const anchorIsDead = !!anchorAfterToggle.person?.death;
        const stillNeedsSpouse = (anchorIsDead && anchorHasInflow) || anchorHasDeadChild;
        ...
        if (node.id === "owner") {
          ...
          if (fixedSpouse && !stillNeedsSpouse) {
