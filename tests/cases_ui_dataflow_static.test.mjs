...
test("unchecking land owner removes auto-generated empty spouse slots", () => {
  ...
  assert.match(toggleBlock[0], /autoGenerated/);
...
test("onMoveWithin materializes auto target slots before moving", () => {
  const moveBlock = reactFlowApp.match(/const onMoveWithin = useCallback\(\(sourceNodeId,\s*targetNodeId\) => \{[\s\S]*?\}, \[commitLogicalNodes, materializeGhostNode, nextId, shareMode\]\);/);
  assert.ok(moveBlock, "onMoveWithin should exist");
...
test("unchecking land owner clears fixed spouse of owner when no inflow and no dead child", () => {
  const toggleBlock = reactFlowApp.match(/const onToggleLandOwner = useCallback\(\(nodeId\) => \{[\s\S]*?\},\s*\[commitLogicalNodes\]\);/);
  assert.ok(toggleBlock, "onToggleLandOwner should exist");
