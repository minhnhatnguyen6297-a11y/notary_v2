...
- Dieu kien bat dau:
  - Codex phai chot ro `batch anh` dang debug.
  - Codex phai chot ro `expected`/ket qua ky vong cho tung anh, tung cap, hoac tung JSON cuoi cung.
  - Neu thieu `batch anh` hoac `expected`, Codex phai hoi lai va dung tai do; chua duoc goi Codex, chua duoc chay UI/project.
- Thu tu bat buoc:
...
- Sau moi task, slash command tu dong cap nhat AGENTS.md section "Lich su chuc nang".

### Relay OCR giua Codex va Codex
- Khi task la `test OCR`/`debug OCR`/`kiem thu OCR`, Codex la ben tu chay vong doi chieu. Codex khong duoc goi Codex de "doan bug" truoc khi da doi chieu xong theo muc `Vong lap kiem thu OCR bat buoc`.
- Codex chi duoc relay sang Codex khi da co du:
  - `batch anh`
  - `expected`
  - `direct output`
  - `project/UI output`
...
- Neu thieu bat ky muc nao o tren, Codex phai tiep tuc tu chay test, hoi user, hoac bao `blocked`; khong duoc mo Claudex run.
...
**[Mo ta]:** Them skill `test-ocr` de ép Codex chay dung vong lap kiem thu OCR bat buoc moi khi gap tu khoa "test OCR", "debug OCR", "OCR sai". Skill chi la trigger/wrapper, source of truth la muc `Vong lap kiem thu OCR bat buoc` trong AGENTS.md.
**[Tech]:**
- File tao moi: `.Codex/skills/test-ocr/SKILL.md` (trigger + dan chieu AGENTS.md)
- File cap nhat: `AGENTS.md` (them rule intent, dieu kien bat dau, decision gate truoc khi goi Codex, guardrail `blocked state`, format bao cao doi chieu), `.Codex/commands/claudex.md` (them gate: OCR task phai co du `batch anh`, `expected`, `direct output`, `project/UI output`, `tang nghi ngo sai` truoc khi draft relay sang Codex)
