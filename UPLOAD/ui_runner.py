...
class App(tk.Tk):
    ...
    def _build_extract_tab(self) -> None:
        ttk.Label(self.extract_tab, text="File hop dong .docx").grid(row=0, column=0, sticky="w")
    ...
    def _build_upload_tab(self) -> None:
        ...
        ttk.Button(button_row, text="Refresh Queue", command=self._refresh_upload_queue).pack(side="left")
        ttk.Button(button_row, text="Start Dry-run", command=self._start_upload_prepare).pack(side="left", padx=(8, 0))
        ttk.Button(button_row, text="Stop", command=self._stop_upload_prepare).pack(side="left", padx=(8, 0))
        ttk.Button(button_row, text="Finalize Selected", command=self._finalize_selected_upload_records).pack(
            side="left", padx=(8, 0)
        )
        ...
        self.upload_tree.grid(row=4, column=0, columnspan=2, sticky="nsew", pady=(8, 0))
        ...
        note = ttk.Label(
            self.upload_tab,
            text="Dry-run se mo tab moi cho tung ho so, dung truoc nut Luu, roi chuyen sang ho so tiep theo den khi dat gioi han tab.",
        )
        note.grid(row=5, column=0, columnspan=2, sticky="w", pady=(12, 0))

        self.upload_tab.columnconfigure(0, weight=1)
        self.upload_tab.rowconfigure(4, weight=1)
    ...
    def _run_batch_from_ui(self) -> None:
        ...
        def run_batch():
            ...
            summary = (
                f"[BATCH] Xong. candidates={manifest['stats']['candidates_found']}, "
    ...
    def _run_extract_from_ui(self) -> None:
        ...
        if not file_value:
            messagebox.showwarning(APP_TITLE, "Vui long chon file .docx.")
            return

        file_path = Path(file_value)
        if not file_path.exists() or file_path.suffix.lower() != ".docx":
            messagebox.showwarning(APP_TITLE, "File da chon khong hop le hoac khong phai .docx.")
            return
