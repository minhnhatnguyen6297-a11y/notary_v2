...
- auto sinh o vo/chong / cha-me + tu sinh nhánh Z.
- 3 trang thai thua ke (unset/accept/refuse) voi 3 nut duoi the.
...
- **Luong 2 (tai san) — HIDDEN:**
  ...
  - ★ toggle da duoc thay bang nut "Chu dat" ben duoi the.
  - Backend tu tinh huong chay: hang 2 ↑ bo me + ↓ con.

### Spouse pairing — auto slot
- `resolveSubRelations()` tu sinh `auto_spouse_{anchorId}` khi anchor la Chu dat, hoac nguoi chet co phan tai san, hoac co con chet.
- Slot auto la node `kind="person"`, `autoGenerated:true`, `person:null`.
- Drop vao slot auto → materialize vao `logicalNodes` (commitAssign/moveWithinDiagram them slot vao prevNodes truoc khi assign).
- Tắt Chủ đất → xoa slot auto trong; neu co nguoi thi tra ve pool roi xoa.

### 3 trang thai thua ke
- 3 nut duoi the: **Chu dat**, **Nhan**, **Tu choi**. `inheritanceDecision: unset|accept|refuse`.
- `refuse` bi loai khoi dong chia; `unset` van tinh suất nhung khong vao participant nhan/tu choi.
- Bo nut `+ Them Anh/Chi/Em` thu cong; Z tu sinh khi cha/me chet sau con va co dong tai san.
...
- Card chi chua: Ten, Ngay sinh–mat, nut ×, 3 nut duoi (Chu dat, Nhan, Tu choi), o %.
...
### Phase 5 — Auto spouse/co-parent slots
- `resolveSubRelations()` tu sinh `auto_spouse_{anchorId}` khi anchor la Chu dat, hoac nguoi chet co phan tai san, hoac co con chet.
- Slot auto la node `kind="person"`, `autoGenerated:true`, `person:null`, hien "Tha nguoi vao day".
- Owner spouse slot an/hien dua tren `shouldShowOwnerSpouse` (isLandOwner || owner chet co share || co con chet).
- O cha/me co dinh (`father/mother/spouse_father/spouse_mother`) la base slot tuong thich, nhung slot trong chi hien khi anchor da co nguoi va co ly do nghiep vu: Chu dat, nguoi chet, co share chay qua, hoac co con chet. Slot da co nguoi thi luon hien de khong mat du lieu.
- Drop vao slot auto → materialize vao `logicalNodes` (commitAssign/moveWithinDiagram them slot vao prevNodes truoc khi assign).
- `createLogicalNode()` phai preserve `autoGenerated` + `autoAnchorId`; neu mat metadata nay thi auto Z/spouse slot validate duoc nhung khong materialize khi drop.
- Tắt Chu đất → recompute `stillNeedsSpouse` từ `resolveSubRelations(nextNodes, shareMode)`; chi xoa/tra pool khi khong con ly do nghiep vu giu spouse.
