...
from services.word_engine import _normalize_token
...
class CompareEngine:
    ...
    def _build_corpus(self) -> list[tuple[str, str, str]]:
        ...
        for span in self.spans:
            ...
            for page_id in span.page_ids:
                ...
                if anchor_found:
                    # Entire page becomes notary-only for compare purposes.
                    continue
    ...
    def _compare_field(self, doc: WordAuditDoc, field: WordField) -> AuditIssue | None:
        ...
        if match_type in (MatchType.EXACT, MatchType.NORMALIZED) and severity == Severity.CERTAIN:
            return None
        if match_type in (MatchType.EXACT, MatchType.NORMALIZED) and severity == Severity.LIKELY:
            return None
    ...
    def _find_best_hit(self, word_value: str, field_name: str) -> _Hit | None:
        ...
        for page_id, text, span_type in self.corpus:
            ...
            # Try exact on compact digits/dates.
            compact_word = re.sub(r"\D", "", word_value)
            compact_text = re.sub(r"\D", "", text)
            if compact_word and compact_word in compact_text:
                score = 100.0
                if best is None or score > best_score:
                    best_score = score
                    best = _Hit(value=word_value, page_refs=[page_id], score=score)
                elif score == best_score:
                    best.page_refs.append(page_id)
                continue
    ...
    def _evaluate(
        ...
        if field_name in self.EXACT_FIELDS:
            ...
            compact_word = re.sub(r"\D", "", word_value)
            compact_ocr = re.sub(r"\D", "", ocr_value)
            if compact_word and compact_word == compact_ocr:
