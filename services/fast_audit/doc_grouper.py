...
from services.fast_audit.rules import classify_doc_type, is_notary_page
...
class DocumentGrouper:
    ...
    def group(self, pages: list[OCRPage]) -> list[DocumentSpan]:
        ...
        for page in pages:
            text = page.text or ""
            notary = is_notary_page(text)
            detected = classify_doc_type(text)
